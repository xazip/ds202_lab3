...
HAPPY <- HAPPY %>% mutate_if(is.character, as.factor) %>% mutate(AGE = as.numeric(levels(AGE)[AGE])) %>% mutate(AGE = as.integer(AGE))
...
HAPPY <- reorder(HAPPY$MARITAL, HAPPY$AGE, FUN = mean, na.rm = TRUE)

#HAPPY


### HAVING TROUBLE WITH THIS BLOCK OF CODE
